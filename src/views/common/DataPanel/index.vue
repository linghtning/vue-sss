...
function onUpdateActiveKey(value: string) {
...
<template>
  <div class="data-panel">
    <a-tabs :active-key="dataLevel" @update:active-key="onUpdateActiveKey">
      <a-tab-pane key="my">
        <template #tab>
    ...
    <a-row class="panel-content">
      <a-col class="main-data-section data-section" :span="8" :xxl="3">
        <div class="icon-box">
          <Icon :name="mainData.icon?.name" :form="mainData.icon?.name" />
        </div>
        <div class="data-item">
          <div class="value">
            {{ mainData.value }}
      ...
      <a-col v-if="!hideStatusSection" class="status-data-section data-section" :span="16" :xxl="5">
        <div v-for="item in statusData" :key="item.title" class="data-item">
          <div class="value">
            {{ item.value }}
          </div>
      ...
      <template v-if="!hideExtraSection">
        <ExtraDataBusinessSystem v-if="extraType === 'businessSystem'" />
        <ExtraDataCommon v-else :extra-data="extraData" />
      </template>
    </a-row>
  </div>
</template>
